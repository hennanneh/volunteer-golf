...
| 2026-05-26 | Pairings tracker moved server-side (was localStorage-only). New `pairings.json` (separate from data.json, gitignored). GET /api/pairings (public, like /api/data); POST /api/pairings rate-limited (30/min), requireAuth(admin/chair/asstChair/captain), shape-validated, empty-body wipe guard (must pass `clear:true` to empty). Viewer fetches from server w/ localStorage as offline cache; check-off marks + startTee stay per-device. Upload page (`/pairings/upload.html?key=…`, client-side obscurity gate only — server auth is the real control) publishes to the endpoint. sw.js bumped to v33. | `6769195` |
