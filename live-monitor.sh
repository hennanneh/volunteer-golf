...
exec tail -n 0 -F "$LOG_ERR" 2>/dev/null | while IFS= read -r line; do
  case "$line" in
    ...
    # Save was blocked because captain's browser tab was older than 10 min
    *"stale dataReadAt"*)
      push "Save blocked — outdated browser tab" \
           "high" "stop_sign" \
           "A captain's tab was older than 10 min. Their changes did NOT save. Ask them to reload the page." ;;

    # Save was blocked because browser is on cached pre-fix code
    *"legacy-mode POST"*)
      push "Save blocked — old cached browser" \
           "high" "stop_sign" \
           "A captain's browser is running cached code from before the 5/14 fix. Ask them to hard-refresh (Ctrl+Shift+R)." ;;

    # Catch-all for other REJECTED save reasons (missing fields, vol count drop, etc.)
    *"REJECTED /api/data"*)
      push "Save blocked" "high" "stop_sign" \
           "A bulk save was rejected. Check the error log for the specific reason." ;;
    ...
    *"AUTH FORBIDDEN"*)
      ...
      push "Unauthorized save attempt${i:+ — $i}" \
           "default" "no_entry" \
           "A logged-in user tried to perform an action they don't have permission for (likely the known broadcast-handler bug)." ;;
    ...
    *"UNAUTH FALLBACK"*)
      push "Old browser detected (legacy auth path)" \
           "low" "grey_question" \
           "Allowed for now, but this captain hasn't refreshed since the auth update." ;;
  esac
done
